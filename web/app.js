(() => {
  ...
  function openAgentRetrievalSettings(){
    ...
    const refreshEmbeddingStatus=async()=>{const model=$('#rag-embedding-model').value.trim(),box=$('#rag-embedding-status');if(!model){box.textContent='Embedding 未配置：当前仍使用多路 FTS5 + RRF + 二次检索。';return}try{const s=await api('/api/retrieval/embedding/status',{method:'POST',body:{model}});box.textContent=`语义索引：${s.embedded_units||0}/${s.total_units||0} 个检索单元${s.complete?' · 已完整':' · 尚未完整'}`}catch(e){box.textContent='语义索引状态：'+e.message}};refreshEmbeddingStatus();
    $('#rag-embedding-build').onclick=async()=>{const model=$('#rag-embedding-model').value.trim();if(!model)return toast('请先填写 Embedding 模型名称',true);if(!confirm('将使用当前 Agent API 配置生成本地检索单元的 Embedding。继续吗？'))return;const b=$('#rag-embedding-build');try{b.disabled=true;b.textContent='构建中…';const s=await api('/api/retrieval/embedding/rebuild',{method:'POST',body:{model,force:false,batch_size:32}});toast(`语义索引完成：本次 ${s.indexed_now||0}，共 ${s.embedded_units||0}/${s.total_units||0}`);await refreshEmbeddingStatus()}catch(e){toast(e.message,true)}finally{b.disabled=false;b.textContent='构建 / 更新'}};
