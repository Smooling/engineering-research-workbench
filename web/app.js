(() => {
  ...
  function openAgentRetrievalSettings(){
    ...
    modal('自动检索 · 本地知识',`<div class="rag-settings"><label class="rag-switch"><input id="rag-enabled" type="checkbox" ${opt.enabled?'checked':''}> <strong>发送问题时自动检索本地研究资料</strong></label><div class="row-meta">第一版不使用 Embedding：FTS5 关键词召回 + 项目/标签/类型过滤 + WikiLink 扩展 + 确定性重排。手动引用仍会优先保留。</div><div class="form-grid" style="margin-top:14px"><div class="field span-2"><label>项目范围</label><select id="rag-project"><option value="">整个 Workspace</option>${(state.projects||[]).map(p=>`<option value="${esc(p)}" ${p===opt.project?'selected':''}>${esc(p)}</option>`).join('')}</select><span class="field-help">选定项目后是硬过滤，其他项目不会进入候选。</span></div><div class="field"><label>最多返回</label><input id="rag-limit" type="number" min="1" max="12" value="${Number(opt.limit||6)}"></div><div class="field"><label>WikiLink 扩展</label><select id="rag-wikilink"><option value="1" ${opt.expand_wikilinks!==false?'selected':''}>启用</option><option value="0" ${opt.expand_wikilinks===false?'selected':''}>关闭</option></select></div><div class="field span-4"><label>文档类型（不选 = 全部）</label><div class="rag-kind-grid">${kinds.map(k=>`<label><input type="checkbox" data-rag-kind value="${k}" ${(opt.kinds||[]).includes(k)?'checked':''}> ${esc(kindLabel(k))}</label>`).join('')}</div></div><div class="field span-4"><label>必须包含的标签（逗号分隔，可留空）</label><input id="rag-tags" value="${esc((opt.tags||[]).join(', '))}" placeholder="例如：ToT, 潮流"><span class="field-help">多个标签按 AND 过滤：文档必须同时具有这些标签。</span></div></div><div id="rag-preview-box" class="rag-preview-box"></div></div>`,`<button class="secondary-btn" id="rag-preview">用当前输入测试检索</button><button class="secondary-btn" id="rag-cancel">取消</button><button class="primary-btn" id="rag-save">应用</button>`);
    const collect=()=>({enabled:$('#rag-enabled').checked,project:$('#rag-project').value,tags:$('#rag-tags').value.split(/[,，]/).map(x=>x.trim()).filter(Boolean),kinds:[...document.querySelectorAll('[data-rag-kind]:checked')].map(x=>x.value),limit:Math.max(1,Math.min(12,+$('#rag-limit').value||6)),expand_wikilinks:$('#rag-wikilink').value==='1'});
    ...
    $('#rag-preview').onclick=async()=>{const q=$('#agent-input')?.value?.trim()||'';const box=$('#rag-preview-box');if(!q){box.innerHTML='<div class="empty compact">请先在 Agent 输入框里写一个问题，再打开这里测试。</div>';return}try{box.innerHTML='<div class="empty compact">正在检索…</div>';const res=await api('/api/retrieval/search',{method:'POST',body:{query:q,options:{...collect(),enabled:true}}});box.innerHTML=(res.items||[]).length?(res.items||[]).map(x=>`<div class="rag-preview-item"><div><strong>#${x.rank} ${esc(x.title)}</strong><span class="badge">${esc(kindLabel(x.kind))}</span><span class="badge accent">score ${esc(x.score)}</span></div><div class="row-meta">${esc(x.heading||'无章节标题')} · ${esc(x.reason||'')}</div><p>${esc((x.passage||'').slice(0,260))}</p></div>`).join(''):'<div class="empty compact">没有找到达到相关性条件的资料。</div>'}catch(e){box.innerHTML=`<div class="empty compact danger">${esc(e.message)}</div>`}}
  }
