(() => {
  ...
  function openAgentRetrievalSettings(){
    ...
    const collect=()=>({enabled:$('#rag-enabled').checked,project:$('#rag-project').value,tags:$('#rag-tags').value.split(/[,，]/).map(x=>x.trim()).filter(Boolean),kinds:$('[data-rag-kind]:checked').map(x=>x.value),limit:Math.max(1,Math.min(12,+$('#rag-limit').value||6)),expand_wikilinks:$('#rag-wikilink').value==='1'});
