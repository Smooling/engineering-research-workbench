(() => {
  ...
  const storedJson = (key, fallback) => { try { const x=JSON.parse(localStorage.getItem(key)||'null'); return x&&typeof x==='object'&&!Array.isArray(x)?x:fallback; } catch { return fallback; } };
  ...
  async function renderAgent(){
    ...
    $('#agent-ref').onclick=openAgentReferencePicker;$('#agent-rag-settings').onclick=openAgentRetrievalSettings;$('#agent-image').onclick=pickAgentImage;$('#agent-delete').onclick=async()=>{if(state.agentSession&&confirm('将该对话移入 Trash？')){await api('/api/agent/sessions/'+encodeURIComponent(state.agentSession),{method:'DELETE'});state.agentSession=null;renderAgent()}};
  ...
  function agentMessageHtml(m){const refs=(m.refs||[]).map(r=>`<span class="badge">手动 · ${esc(r.title)}</span>`).join('');const autoRefs=(m.auto_refs||[]).map(r=>`<span class="badge accent agent-auto-ref" title="${esc(r.reason||'自动检索')} · score ${esc(r.score??'')}">自动 · ${esc(r.title)}${r.heading?` / ${esc(r.heading)}`:''}</span>`).join('');const imgs=(m.images||[]).map(p=>`<img src="/workspace-file/${esc(typeof p==='string'?p:(p.path||''))}" alt="对话图片">`).join('');const showReasoning=state.config?.app?.llm?.show_reasoning!==false;const reasoning=showReasoning&&String(m.reasoning||'').trim()?`<details class="agent-reasoning"><summary>模型思考过程</summary><div class="agent-reasoning-body" data-raw="${esc(m.reasoning||'')}"></div></details>`:'';return `<article class="agent-message ${m.role==='assistant'?'assistant':'user'}"><div class="agent-avatar">${m.role==='assistant'?'AI':'YOU'}</div><div class="agent-bubble">${(refs||autoRefs)?`<div class="agent-msg-refs">${refs}${autoRefs}</div>`:''}${imgs?`<div class="agent-msg-images">${imgs}</div>`:''}${reasoning}<div class="agent-message-body" data-raw="${esc(m.content||'')}"></div><div class="agent-msg-meta">${esc(m.model||'')}${m.request_preset_label?` · ${esc(m.request_preset_label)}`:''} · ${esc(fmtTime(m.created))}</div></div></article>`}
  ...
  function paintAgentContext(){const root=$('#agent-context-strip');if(!root)return;const manual=state.agentRefs.length?`<span class="row-meta">手动引用</span>${state.agentRefs.map((r,i)=>`<button type="button" class="context-chip" data-ref-remove="${i}">${esc(r.title)}</button>`).join('')}`:'<span class="row-meta">未手动引用</span>';const rag=state.agentRetrieval.enabled?`<span class="badge accent">自动检索开启${state.agentRetrieval.project?' · '+esc(state.agentRetrieval.project):' · 全 Workspace'}${state.agentRetrieval.kinds?.length?' · '+state.agentRetrieval.kinds.map(kindLabel).join('/') :''}</span>`:'<span class="badge">自动检索关闭</span>';root.innerHTML=manual+rag;[...root.querySelectorAll('[data-ref-remove]')].forEach(b=>b.onclick=()=>{state.agentRefs.splice(+b.dataset.refRemove,1);paintAgentContext()})}
  ...
  function saveAgentRetrievalOptions(){localStorage.setItem('agentRetrievalOptions',JSON.stringify(state.agentRetrieval));paintAgentContext()}
  function openAgentRetrievalSettings(){
    const opt=state.agentRetrieval||{};const kinds=['idea','journal','note','milestone','summary','literature'];
    modal('自动检索 · 本地知识',`<div class="rag-settings"><label class="rag-switch"><input id="rag-enabled" type="checkbox" ${opt.enabled?'checked':''}> <strong>发送问题时自动检索本地研究资料</strong></label><div class="row-meta">Recall v2：Multi-Query + Document/Section/Chunk 三级召回 + Metadata + WikiLink + RRF + Coverage Check + 自适应第二轮；Embedding 为可选通道。</div><div class="form-grid" style="margin-top:14px"><div class="field span-2"><label>项目范围</label><select id="rag-project"><option value="">整个 Workspace</option>${(state.projects||[]).map(p=>`<option value="${esc(p)}" ${p===opt.project?'selected':''}>${esc(p)}</option>`).join('')}</select><span class="field-help">项目是否作为硬过滤由右侧“项目策略”决定；推荐使用“项目优先”。</span></div><div class="field"><label>项目策略</label><select id="rag-project-mode"><option value="prefer" ${(opt.project_mode||'prefer')==='prefer'?'selected':''}>项目优先</option><option value="strict" ${opt.project_mode==='strict'?'selected':''}>严格限定</option><option value="all" ${opt.project_mode==='all'?'selected':''}>全 Workspace</option></select></div><div class="field"><label>最终返回</label><input id="rag-limit" type="number" min="1" max="20" value="${Number(opt.limit||8)}"></div><div class="field"><label>候选池</label><input id="rag-candidate-limit" type="number" min="20" max="200" value="${Number(opt.candidate_limit||80)}"></div><div class="field"><label>WikiLink 扩展</label><select id="rag-wikilink"><option value="1" ${opt.expand_wikilinks!==false?'selected':''}>启用</option><option value="0" ${opt.expand_wikilinks===false?'selected':''}>关闭</option></select></div><div class="field"><label>Multi-Query</label><select id="rag-multi"><option value="1" ${opt.multi_query!==false?'selected':''}>启用</option><option value="0" ${opt.multi_query===false?'selected':''}>关闭</option></select></div><div class="field"><label>覆盖不足自动二检</label><select id="rag-second"><option value="1" ${opt.adaptive_second_pass!==false?'selected':''}>启用</option><option value="0" ${opt.adaptive_second_pass===false?'selected':''}>关闭</option></select></div><div class="field"><label>Embedding 语义召回</label><select id="rag-embedding-enabled"><option value="0" ${!opt.embedding_enabled?'selected':''}>关闭</option><option value="1" ${opt.embedding_enabled?'selected':''}>启用</option></select><span class="field-help">服务地址与模型统一在“设置 → Embedding”配置。</span></div><div class="field span-4"><label>文档类型（不选 = 全部）</label><div class="rag-kind-grid">${kinds.map(k=>`<label><input type="checkbox" data-rag-kind value="${k}" ${(opt.kinds||[]).includes(k)?'checked':''}> ${esc(kindLabel(k))}</label>`).join('')}</div></div><div class="field span-4"><label>必须包含的标签（逗号分隔，可留空）</label><input id="rag-tags" value="${esc((opt.tags||[]).join(', '))}" placeholder="例如：ToT, 潮流"><span class="field-help">多个标签按 AND 过滤：文档必须同时具有这些标签。</span></div></div><div id="rag-embedding-status" class="row-meta" style="margin-top:10px"></div><div id="rag-preview-box" class="rag-preview-box"></div></div>`,`<button class="secondary-btn" id="rag-preview">用当前输入测试检索</button><button class="secondary-btn" id="rag-cancel">取消</button><button class="primary-btn" id="rag-save">应用</button>`);
    const collect=()=>({enabled:$('#rag-enabled').checked,project:$('#rag-project').value,project_mode:$('#rag-project-mode').value,tags:$('#rag-tags').value.split(/[,，]/).map(x=>x.trim()).filter(Boolean),kinds:[...document.querySelectorAll('[data-rag-kind]:checked')].map(x=>x.value),limit:Math.max(1,Math.min(20,+$('#rag-limit').value||8)),candidate_limit:Math.max(20,Math.min(200,+$('#rag-candidate-limit').value||80)),expand_wikilinks:$('#rag-wikilink').value==='1',multi_query:$('#rag-multi').value==='1',adaptive_second_pass:$('#rag-second').value==='1',embedding_enabled:$('#rag-embedding-enabled').value==='1',embedding_model:''});
    $('#rag-cancel').onclick=closeModal;$('#rag-save').onclick=()=>{state.agentRetrieval=collect();saveAgentRetrievalOptions();closeModal();renderAgent()};
    $('#rag-preview').onclick=async()=>{const q=$('#agent-input')?.value?.trim()||'',box=$('#rag-preview-box');if(!q){box.innerHTML='<div class="empty compact">请先在 Agent 输入框里写一个问题，再打开这里测试。</div>';return}try{box.innerHTML='<div class="empty compact">正在进行多路检索…</div>';const res=await api('/api/retrieval/search',{method:'POST',body:{query:q,options:{...collect(),enabled:true}}});const meta=res.meta||{},cov=meta.coverage||{},first=meta.first_pass_coverage||{};const diag='<div class="rag-diagnostic"><strong>检索诊断</strong><span>Query variants：'+(res.query_variants||[]).length+'</span><span>候选池：'+(meta.candidate_count||0)+'</span><span>第一轮覆盖：'+esc(first.ratio??'—')+'</span><span>最终覆盖：'+esc(cov.ratio??'—')+'</span><span>第二轮：'+(meta.second_pass?'已触发':'未触发')+'</span>'+((meta.feedback_terms||[]).length?'<span>扩展词：'+esc(meta.feedback_terms.join(' / '))+'</span>':'')+(meta.embedding?.enabled?'<span>Embedding：'+esc(meta.embedding.model||'')+' · indexed '+(meta.embedding.indexed_units||0)+'</span>':'')+'<details><summary>查看 Multi-Query</summary><div class="rag-query-list">'+(res.query_variants||[]).map(x=>'<code>'+esc(x)+'</code>').join('')+'</div></details></div>';const rows=(res.items||[]).length?(res.items||[]).map(x=>'<div class="rag-preview-item"><div><strong>#'+x.rank+' '+esc(x.title)+'</strong><span class="badge">'+esc(x.kind_label||kindLabel(x.kind))+'</span><span class="badge">'+esc(x.level||'')+'</span><span class="badge accent">score '+esc(x.score)+'</span></div><div class="row-meta">'+esc(x.heading||'文档级')+' · coverage '+esc(x.coverage??'—')+'</div><div class="rag-route-line">'+(x.routes||[]).slice(0,7).map(t=>'<span>'+esc(t.route)+' #'+esc(t.rank)+'</span>').join('')+'</div><div class="row-meta">'+esc(x.reason||'')+'</div><p>'+esc((x.passage||'').slice(0,360))+'</p></div>').join(''):'<div class="empty compact">没有找到候选资料。</div>';box.innerHTML=diag+rows}catch(e){box.innerHTML='<div class="empty compact danger">'+esc(e.message)+'</div>'}}
  }
  ...
  async function sendAgentMessage(){
    ...
    try{
      ...
      try{const r=await api('/api/agent/send',{method:'POST',body:{session_id:state.agentSession,message:text,refs:refs.map(x=>x.id),images:images.map(x=>x.path),request_preset:preset,retrieval:state.agentRetrieval}});state.agentSession=r.session.id;clearInterval(timer);await renderAgent()}catch(e){clearInterval(timer);$('#agent-thinking')?.remove();if(root){root.insertAdjacentHTML('beforeend',`<article class="agent-message assistant"><div class="agent-avatar">AI</div><div class="agent-bubble"><div class="agent-error">请求失败：${esc(e.message)}</div></div></article>`);root.scrollTop=root.scrollHeight}toast(e.message,true)}
  ...
  function renderSettingsTab(tab,cfg){const p=$('#settings-panel'),app=cfg.app,rss=cfg.rss;if(tab==='general'){
      ...
      const emb=app.embedding||{};
      p.innerHTML=`<div class="card-head"><div><div class="card-kicker">RAG / EMBEDDING</div><h3>Embedding 与语义索引</h3><p class="row-meta">独立于 Agent / Chat API。API Key 只保存在本机 config/secret.json，不进入 Workspace 或公开配置。</p></div><span class="badge ${emb.has_api_key?'accent':'warn'}">${emb.has_api_key?'API Key 已保存':'API Key 未配置'}</span></div><div class="form-grid"><div class="field"><label>启用 Embedding</label><select id="emb-enabled"><option value="1" ${emb.enabled?'selected':''}>启用</option><option value="0" ${!emb.enabled?'selected':''}>关闭</option></select></div><div class="field span-2"><label>Base URL</label><input id="emb-base" value="${esc(emb.base_url||'https://api.openai.com/v1')}" placeholder="https://api.openai.com/v1"></div><div class="field span-2"><label>API Key</label><input id="emb-key" type="password" value="" placeholder="${emb.has_api_key?'已保存；留空则保持不变':'请输入 Embedding API Key'}"><span class="field-help">Key 仅保存到本机 config/secret.json，不写入 app.json、Workspace 或 Git。</span></div><div class="field span-2"><label>Embedding 模型</label><input id="emb-model" value="${esc(emb.model||'')}" placeholder="例如：text-embedding-v4"></div><div class="field"><label>Batch Size</label><input id="emb-batch" type="number" min="1" max="128" value="${Number(emb.batch_size||32)}"></div><div class="field"><label>超时 / 秒</label><input id="emb-timeout" type="number" min="5" max="600" value="${Number(emb.timeout||120)}"></div></div><div id="emb-index-status" class="row-meta" style="margin-top:12px"></div><div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap"><button class="primary-btn" id="emb-save">保存 Embedding 设置</button><button class="secondary-btn" id="emb-test">测试连接</button><button class="secondary-btn" id="emb-build">构建 / 更新语义索引</button></div>`;
      const collectEmbedding=()=>({enabled:$('#emb-enabled').value==='1',base_url:$('#emb-base').value.trim(),api_key:$('#emb-key').value.trim(),model:$('#emb-model').value.trim(),batch_size:Math.max(1,Math.min(128,+$('#emb-batch').value||32)),timeout:Math.max(5,Math.min(600,+$('#emb-timeout').value||120))});
      const refreshEmbStatus=async()=>{const box=$('#emb-index-status');const model=$('#emb-model').value.trim();if(!model){box.textContent='尚未配置模型。';return}try{const s=await api('/api/retrieval/embedding/status',{method:'POST',body:{model}});box.textContent=`语义索引：${s.embedded_units||0}/${s.total_units||0} 个检索单元${s.complete?' · 已完整':' · 尚未完整'}`}catch(e){box.textContent='语义索引状态：'+e.message}};
      refreshEmbStatus();
      $('#emb-save').onclick=async()=>{app.embedding=collectEmbedding();const saved=await api('/api/config/app',{method:'POST',body:app});state.config.app=saved;cfg.app=saved;toast('Embedding 设置已保存')};
      $('#emb-test').onclick=async()=>{try{$('#emb-test').disabled=true;app.embedding=collectEmbedding();const saved=await api('/api/config/app',{method:'POST',body:app});state.config.app=saved;cfg.app=saved;const x=await api('/api/retrieval/embedding/test',{method:'POST',body:{}});toast(`连接成功 · ${x.model} · ${x.dimension} 维`)}catch(e){toast(e.message,true)}finally{$('#emb-test').disabled=false}};
      $('#emb-build').onclick=async()=>{if(!confirm('将使用独立 Embedding API 对新增或变更的本地检索单元生成向量。继续吗？'))return;try{$('#emb-build').disabled=true;app.embedding=collectEmbedding();const saved=await api('/api/config/app',{method:'POST',body:app});state.config.app=saved;cfg.app=saved;const x=await api('/api/retrieval/embedding/rebuild',{method:'POST',body:{model:'',force:false,batch_size:app.embedding.batch_size}});toast(`语义索引完成：本次 ${x.indexed_now||0}，共 ${x.embedded_units||0}/${x.total_units||0}`);await refreshEmbStatus()}catch(e){toast(e.message,true)}finally{$('#emb-build').disabled=false}};
    }
